...
class DashboardController extends Controller
{
    ...
    public function viewCompletedtopics($userid,$id)
    {
        ...
        if ($id == 1) {
            ...
            return view('Admin.topics.topic_0', compact('topics','userid'));
        } elseif ($id == 2) {
            $topics = Topic::where('id', $id)->with('questions')->first();

            return view('Admin.topics.topic_1', compact('topics','userid'));
        } elseif ($id==4) {
            $topics = Topic::where('id', $id)->with('questions')->first();

            return view('Admin.topics.topic_2', compact('topics','userid'));
        }elseif ($id==6) {
            $topics = Topic::where('id', $id)->with('questions')->first();

            return view('Admin.topics.topic_3', compact('topics','userid'));
        } elseif ($id==7) {
            $topics = Topic::where('id', $id)->with('questions')->first();

            return view('Admin.topics.topic_4', compact('topics','userid'));
        } elseif ($id==11) {
            $topics = Topic::where('id', $id)->with('questions')->first();

            return view('Admin.topics.topic_5', compact('topics','userid'));
        } elseif ($id==12) {
            $topics = Topic::where('id', $id)->with('questions')->first();

            return view('Admin.topics.topic_6', compact('topics','userid'));
        } elseif ($id==13) {
            $topics = Topic::where('id', $id)->with('questions')->first();
