...
class DashboardController extends Controller
{
    ...
    public function viewCompletedtopics($userid,$id)
    {
        ...
        if ($id == 1) {
            ...
            return view('Practitioner.topics.topic_5', compact('topics','userid'));
        } elseif ($id==12) {
            $topics = Topic::where('id', $id)->with('questions')->first();

            return view('Practitioner.topics.topic_6', compact('topics','userid'));
        } elseif ($id==13) {
            $topics = Topic::where('id', $id)->with('questions')->first();

            return view('Practitioner.topics.topic_7', compact('topics','userid'));
        } elseif ($id==14) {
            $topics = Topic::where('id', $id)->with('questions')->first();

            return view('Practitioner.topics.topic_8', compact('topics','userid'));
        } 
    }
}
